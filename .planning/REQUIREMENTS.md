...
- [x] **SETUP-02**: A "hello world" page is reachable at `https://<user>.github.io/battery-calculator/` via GitHub Actions deploy (`actions/checkout@v6` + `setup-node@v6` + `configure-pages@v6` + `upload-pages-artifact@v5` + `deploy-pages@v5`)
- [x] **SETUP-03**: CI runs lint, formatter check, and Vitest on every push; deploy only runs after CI is green on the main branch
- [x] **SETUP-04**: No third-party scripts ship in the bundle (no analytics, no Sentry, no Google Fonts, no CDN-loaded libraries) — verified by a CI grep / Network-tab inspection note in the README
...
- [x] **PRIV-03**: Parse errors are shown to the user but never sent off-device (no error-reporting library)
